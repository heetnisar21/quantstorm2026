...
# DYNAMIC SHADING: Aggressive in late rounds to secure sure positive EV trades.
# Early rounds: shade heavily (noise protection). Late rounds: bid nearly full value.
SHADE_BY_ROUND = {1: 0.60, 2: 0.70, 3: 0.80, 4: 0.90, 5: 0.95}

# Calibrated tick value of each power, per round
POWER_VALUES = {
...
TRANSFORM_THRESHOLD = 0.5  # Only use Transform if hand is highly neutral
Z_FORCE = 0.55             # Slightly safer SE multiplier for the forcing margin
...
class Bot:
    ...
    # =============================================
    # VALUE ESTIMATION (Kept from v4 - Robust)
    # =============================================
    ...
    def _estimate_S(self, obs, live_quote=None):
        ...
        components = [(0.0, float(n_private))]  # (mean, variance) of opponent
    ...
    def bid(self, obs, offered: list) -> dict:
        ...
        values = {}
        for name in offered:
            # 1. DYNAMIC SHADE: Use round-based shade instead of flat 0.6
        ...
        # Apply the dynamic shading here
        ...
        # Budget too tight: fund highest-value powers first
        budget = obs.te_mine
        out = {}
    ...
    def _transform_value(self, obs):
        ...
        # 2. TIGHTER TRANSFORM THRESHOLD: Only use if hand is strictly neutral
        if abs(obs.k_mine) <= TRANSFORM_THRESHOLD:
            return swap
        return 0.0

    # =============================================
    # NEGOTIATION (MINOR UPGRADES)
    # =============================================
    ...
    def respond(self, obs, quote: tuple, turn: int):
        ...
        # 3. CONFIG-BASED LOSS CAP: Use game config instead of hardcoded -2.0
